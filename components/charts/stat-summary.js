...
export const StatSummary = ({ data, total }) => {
  const dataInfo = [
  ...
  const denominator = appointment + consultation;
      ...
      <div className="flex justify-center gap-16">
        <div className="flex flex-col gap-1">
          ...
          <h2 className="text-xs text-gray-400">
            {dataInfo[1].name}(
            {denominator > 0
              ? ((appointment / denominator) * 100).toFixed(0)
              : 0}
            )
          </h2>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <h2 className="text-xs text-gray-400">
            {dataInfo[2].name}(
            {denominator > 0
              ? ((consultation / denominator) * 100).toFixed(0)
              : 0}
            )
          </h2>
        </div>
